...
A faster XS implementation may be used if L<Sub::HandlesVia::XS> is installed.
...
=head2 C<< for_each2( $coderef ) >>

Arguments: B<< CodeRef >>.

The same as foreach, but the element will only be available as C<< $_ >>: no parameters are passed to the coderef. It may be faster than for_each.

  my $object = My::Class->new( attr => [ 'foo', 'bar', 'baz' ] );
  my $i = 0;
  $object->my_for_each2( sub { say "Item $i is $_."; ++$i } );
...
Returns the first item on the array.

  my $object = My::Class->new( attr => [ 'foo', 'bar', 'baz' ] );
  say $object->my_peek; ## ==> 'foo'

A faster XS implementation may be used if L<Sub::HandlesVia::XS> is installed.

=head2 C<< peekend() >>

Returns the last item on the array.

  my $object = My::Class->new( attr => [ 'foo', 'bar', 'baz' ] );
  say $object->my_peekend; ## ==> 'baz'
...
Like C<< List::Util::reduce() >>, except C<< $a >> and C<< $b >> are passed as parameters.

=head2 C<< reductions( $coderef ) >>

Arguments: B<< CodeRef >>.

Like C<< List::Util::reductions() >>, except C<< $a >> and C<< $b >> are passed as parameters.
...
Like C<sort> from L<perlfunc>, except C<< $a >> and C<< $b >> are passed as parameters.
...
Like C<sort> from L<perlfunc>, but changes the attribute to point to the newly sorted array. C<< $a >> and C<< $b >> are passed as parameters.
